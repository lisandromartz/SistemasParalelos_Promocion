...
void *funcion(void *arg)
{
    ...
    mergeSort_iterative(sorted_slicesA[id], slice, tempA[id]);
    mergeSort_iterative(sorted_slicesB[id], slice, tempB[id]);
    ...
    {
        merge(sorted_slicesA[id], sorted_slicesA[id + merge_threads], slice, tempA[id]);

        merge(sorted_slicesB[id], sorted_slicesB[id + merge_threads], slice, tempB[id]);

        slice *= 2;
        for (i = 0; i < slice; i++)
        {
            sorted_slicesA[id][i] = tempA[id][i];
            sorted_slicesB[id][i] = tempB[id][i];
        }
    ...
    if (id == 0)
    {
        ...
        for (i = 0; i < slice; i++)
        {
            arrA[begin + i] = tempA[id][i];
            arrB[begin + i] = tempB[id][i];
        }
    }
...
void init()
{
    ...
    tempA = (int **) malloc(sizeof(int *) * NUM_THREADS);
    if (tempA == NULL)
    {
        perror("Failed to allocate memory for tempA");
        exit(EXIT_FAILURE);
    }

    tempB = (int **) malloc(sizeof(int *) * NUM_THREADS);
    if (tempB == NULL)
    {
        perror("Failed to allocate memory for tempB");
        exit(EXIT_FAILURE);
    }
    ...
    for (i = 0; i < NUM_THREADS; i++)
    {
        ...
        tempA[i] = (int *) malloc(sizeof(int) * max_size);
        if (tempA[i] == NULL)
        {
            perror("Failed to allocate memory for tempA[i]");
            exit(EXIT_FAILURE);
        }

        tempB[i] = (int *) malloc(sizeof(int) * max_size);
        if (tempB[i] == NULL)
        {
            perror("Failed to allocate memory for tempB[i]");
            exit(EXIT_FAILURE);
        }
    }
